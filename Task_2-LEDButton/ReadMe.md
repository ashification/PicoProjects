# Task 2 - LED Buttons <br/><br/> 
## Hardware
For this task you will need an LED, a 220Ω resistor and 2 buttons from your pack along with the breadboard and pico
![Parts Requires](https://github.com/ashification/PicoProjects/blob/main/Task_2-LEDButton/task%202%20equipment.png)
<br/> <br/> 

Take your previous circuit from task 1 and add your 2 buttons to the board like so:
![Diagram for breadboard assembly](https://github.com/ashification/PicoProjects/blob/main/Task_2-LEDButton/Task%202%20LED%20Button.png)
<br/><br/> 

## Wokwi
For Wokwi you will need to build your board and add your components. For each section the full soloution will be provided (python code and JSON for diagram) both in the repository here as well as a working wokwi project.
Do try it yourself first and go to the examples as references if you get stuck  
- [Task 2 JSON code for Wokwi Diagram ](https://github.com/ashification/PicoProjects/blob/main/Task_2-LEDButton/wokwi_task2_diagram.json)
- [Task 2 Wokwi Full Soloution](https://wokwi.com/projects/462838825376140289)

<br/><br/> 
